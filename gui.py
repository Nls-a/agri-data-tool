...
class App(tk.Tk):
    ...
    def _build_content(self, parent):
        ...
        controls_frame.grid(row=7, column=0, columnspan=3, sticky="ew", pady=(0, 8))
        ttk.Label(controls_frame, text="Number of classes:").pack(side="left")

        self.class_count_var = tk.IntVar(value=5)
        ...
        self.count_spinbox.pack(side="left", padx=(8, 4))
        self.count_spinbox.bind("<<Increment>>", lambda e: self._on_class_count_changed())
        self.count_spinbox.bind("<<Decrement>>", lambda e: self._on_class_count_changed())

        ttk.Button(controls_frame, text="Apply", command=self._on_class_count_changed).pack(side="left", padx=2)
        ttk.Button(controls_frame, text="Add Class", command=self._add_class).pack(side="left", padx=2)
        ttk.Button(controls_frame, text="Remove Class", command=self._remove_class).pack(side="left", padx=2)

        # ── Class ranges (injected here after column is picked) ────
        self.classes_frame = ttk.Frame(parent)
        self.classes_frame.grid(row=8, column=0, columnspan=3, sticky="ew")
        self.classes_frame.columnconfigure(1, weight=1)
        self.classes_frame.columnconfigure(3, weight=1)
        self.classes_frame.columnconfigure(5, weight=1)

        # ── Run ────────────────────────────────────
        self.run_button = ttk.Button(parent, text="Run", command=self._on_run)
        self.run_button.grid(row=9, column=1, sticky="e", padx=(8, 4), pady=(12, 0))

    # ─────────────────────────────────────────
    # CLASS RANGE BUILDER
    # ─────────────────────────────────────────
    def _build_class_ranges(self, min_val, max_val, n_classes=None):
        """Draw editable From/To/Rate rows for each class."""
        if n_classes is None:
            n_classes = self.class_count_var.get()

        # Clear previous entries
        for widget in self.classes_frame.winfo_children():
            widget.destroy()

        self.class_entries = []  # list of (from_var, to_var, rate_var)
        ...
        ttk.Label(self.classes_frame, text="Rate", style="Sub.TLabel").grid(
            row=0, column=5, padx=(8, 0))

        for i in range(n_classes):
            from_var = tk.StringVar(value=str(bins[i]))
            to_var   = tk.StringVar(value=str(bins[i + 1]))
            rate_var = tk.StringVar(value="")   # empty by default
            ...
            ttk.Entry(self.classes_frame, textvariable=rate_var, width=10).grid(
                row=i + 1, column=5, padx=(8, 0), sticky="ew")

            self.class_entries.append((from_var, to_var, rate_var))

        # Expand the window to fit
        self.update_idletasks()
        self.geometry(f"700x{self.winfo_reqheight() + 20}")

    def _get_bins(self):
        """Read class entries and return a sorted list of bin edges."""
        try:
            bins = [float(self.class_entries[0][0].get())]
            for _, to_var, _ in self.class_entries:
    ...
    def _get_rates(self):
        """Read rate values from class entries. Returns list of floats or None if any are empty or invalid."""
        rates = []
        try:
            for _, _, rate_var in self.class_entries:
                rate_str = rate_var.get().strip()
                if rate_str == "":
                    self.set_status("All rate fields must be filled with a number.")
                    return None
                rates.append(float(rate_str))
            return rates
        except ValueError:
            self.set_status("Invalid rate value — please enter numbers only.")
            return None

    def _on_class_count_changed(self, *args):
        """Rebuild classes when count changes, using current column min/max."""
        if hasattr(self, "_gdf") and self.column_var.get():
            col = self.column_var.get()
            col_data = self._gdf[col].dropna()
            self._build_class_ranges(col_data.min(), col_data.max())
        else:
            self.set_status("Load data and select a column first.")

    def _add_class(self):
        """Increase class count by 1 and rebuild."""
        current = self.class_count_var.get()
        self.class_count_var.set(current + 1)
        self._on_class_count_changed()

    def _remove_class(self):
        """Decrease class count by 1 (minimum 1) and rebuild."""
        current = self.class_count_var.get()
        if current > 1:
            self.class_count_var.set(current - 1)
            self._on_class_count_changed()
        else:
            self.set_status("At least one class is required.")
